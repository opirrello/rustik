...
var initializeTabs = function (tabs) {
  ...
  var tabClicked = function (e) {
    ...
    var $target = $content.find('[data-idx=' + targetIdx + ']').first();
...
var dateFieldTemplate = function (type) {
}

var inputFieldTemplate = function (type, extraOpts) {
  var templ = {
    'template':
      '<%= (node.schemaElement.type == "money" ? "<span class=\'with-currency money-" + id + "\'>":"") %>' +
      '<input type="' + type + '" ' +
      ...
      '<%= (node.schemaElement && node.schemaElement.minimum && (node.schemaElement.type == "number" || node.schemaElement.type == "integer") ? " min=\'" + node.schemaElement.minimum + "\'" : "") %>' +
      '<%= (node.schemaElement && node.schemaElement.maximum && (node.schemaElement.type == "number" || node.schemaElement.type == "integer") ? " max=\'" + node.schemaElement.maximum + "\'" : "") %>' +
      '<%= (node.schemaElement && node.schemaElement.step ? " step=\'" + node.schemaElement.step + "\'" : "") %>' +
      '<%= (node.schemaElement && node.schemaElement.required && (node.schemaElement.type !== "boolean") ? " required=\'required\'" : "") %>' +
      '<%= (node.dir? "dir=" + \'"\' + escape(node.dir) + \'"\' : "")%>' +
      '<%= (node.placeholder? "placeholder=" + \'"\' + escape(node.placeholder) + \'"\' : "")%>' +
      '<%= (node.rel? "rel=" + \'"\' + escape(node.rel) + \'"\' : "")%>' +
      '<% if (node.schemaElement.iconify) { %>' + 'style="padding-<%= node.schemaElement.iconify.position%>:36px"' + '<% } %>'+
      ' />'+
      '<%= (node.schemaElement.type == "money" ? "</span>":"") %>' +
      '<% if (node.schemaElement.iconify) { %>' +
      '<div class="form-control-feedback" style="<%= (node.schemaElement.iconify.position == "left")?"right":"left"%>:auto; <%=escape(node.schemaElement.iconify.position) %>:0; line-height:28px; height:28px;">'+
		'<i class="<%= escape(node.schemaElement.iconify.icon) %> text-muted"></i>'+
		'</div>'+
      '<% } %>' ,
    'fieldtemplate': true,
    'inputfield': true
...
jsonform.elementTypes = {
  ...
  'money': inputFieldTemplate('money'),
  ...
  'color':{
    ...
    'onInsert': function(evt, node) {
      $(node.el).find('#' + escapeSelector(node.id)).spectrum({
        preferredFormat: "hex",
        openPalette: (!node.schemaElement.openPalette) ? false : node.schemaElement.openPalette,
        showInput: true,
        color: (node.formElement.initial) ? node.formElement.initial : false,
        chooseText:   (node.formElement.chooseText) ? node.formElement.chooseText : false,
        cancelText:   (node.formElement.cancelText) ? node.formElement.cancelText : false,
        chooseAsLink: (node.formElement.chooseAsLink) ? node.formElement.chooseAsLink : false
      });
    }
  },
  'textarea':{
  ...
  'checkbox':{
    'template': '<label class="checkbox"><input type="checkbox" id="<%= id %>" ' +
      ...
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.formElement.class? " class=\'"+node.formElement.class+"\'" : "")%>' +
      '<%= (node.schemaElement && node.schemaElement.required && (node.schemaElement.type !== "boolean") ? " required=\'required\'" : "") %>' +
  ...
  'select':{
    'template':'<select name="<%= node.name %><%= (node.formElement && node.formElement.multiple ?"[]":"")%>" id="<%= id %>"' +
      '<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'" : "") %>' +
      '<%= (node.disabled? " disabled" : "")%>' +
      '<%= (node.formElement && node.formElement.multiple ? " multiple=\'multiple\'" : "") %>' +
      '<%= (node.schemaElement && node.schemaElement.required ? " required=\'required\'" : "") %>' +
      '> ' +
      '<% if (node.formElement.multiple) { %>'+
      '<% _.each(node.options, function(key, val) { if(key instanceof Object) { if (Array.isArray(value) && _.contains(value, key.value)) { %> <option selected value="<%= key.value %>"><%= key.title %></option> <% } else { %> <option value="<%= key.value %>"><%= key.title %></option> <% }} else { if (Array.isArray(value) && _.contains(value, key)) { %> <option selected value="<%= key %>"><%= key %></option> <% } else { %><option value="<%= key %>"><%= key %></option> <% }}}); %> ' +
      '<% } else { %>'+
  ...
  'radiobuttons': {
    'template': '<div id="<%= node.id %>"<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'" : "") %>>' +
      '<% _.each(node.options, function(key, val) { %>' +
        '<label class="radio btn<% if (((key instanceof Object) && (value === key.value)) || (value === key)) {%> active<% } %>">' +
  ...
  'submit':{
    'template':'<input type="submit" <% if (id) { %> id="<%= id %>" <% } %> class="btn btn-primary <%= elt.htmlClass?elt.htmlClass:"" %>" value="<%= value || node.title %>"<%= (node.disabled? " disabled" : "")%><%= (node.formElement.rel? " rel=" + \'"\' + escape(node.formElement.rel) + \'"\' : "")%>/>'
  },
  'button':{
    'template':' <button <% if (id) { %> id="<%= id %>" <% } %> class="btn <%= elt.htmlClass?elt.htmlClass:"" %>"<%= (node.disabled? " disabled" : "")%><%= (node.formElement.rel? " rel=" + \'"\' + escape(node.formElement.rel) + \'"\' : "")%>><%= node.title %></button> '
  },
  'actions':{
  ...
  'selectfieldset': {
    'template': '<fieldset class="tab-container <%= elt.htmlClass?elt.htmlClass:"" %>"<%= elt.eltId?" id=" + \'"\' + elt.eltId +  \'"\':"" %>>' +
    ...
    'onBeforeRender': function (data, node) {
      ...
      if (node.schemaElement) {
        ...
        node.value = node.schemaElement['default'] || node.value;
      }
      if (node.options) {
        ...
        children = _.map(node.children, function (child, idx) {
           return {
              ...
              value: child.formElement.optValue || choices[child.childPos] || child.childPos,
              node: child
           };
        });
      }
      ...
      var elt = node.formElement;
      var tabs = '<select class="nav"' + (node.name ? ' name="'+node.name+'"' : '') +
        (node.disabled ? ' disabled' : '') +
        '>';
      _.each(children, function (child, idx) {
        tabs += '<option'+
          (child.node.active ? ' selected=true' : '') +
         ' data-idx="' + idx + '" value="' + child.value + '"' +
  ...
  'section': {
    'template': '<div class="section-group<% if (node.formElement.htmlClass) { %> <%=node.formElement.htmlClass%><% } %><% if (node.formElement.required) { %> jsonform-required<% } %>"' +
      '<% if (node.id) { %> id="<%= node.id %>"<% } %>' +
      '><% if (node.title) { %> <label class="control-label"><%= node.title %></label><% } %><%= children %></div>',
